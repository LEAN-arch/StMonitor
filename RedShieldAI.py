...
# actionable intelligence, and operational realism. Features an XGBoost demand model,
...
def find_nearest_node(graph: nx.Graph, point: Point) -> Any:
    """Finds the nearest graph node to a given Shapely Point."""
    # Note: graph 'pos' is stored as [lat, lon], while Shapely Point is (lon, lat)
...
class DataFusionFabric:
    ...
    def _build_road_graph(_self, network_config: Dict) -> nx.Graph:
        """Constructs a NetworkX graph from the configuration. Nodes are stored as [lat, lon]."""
        G = nx.Graph()
        for node, data in network_config.get('nodes', {}).items():
            G.add_node(node, pos=data['pos'])
        for edge in network_config.get('edges', []):
            G.add_edge(edge[0], edge[1], weight=edge[2]) # weight is base travel time/cost
        return G

    @st.cache_data(ttl=60)
    def get_live_state(_self) -> Dict:
        """Simulates fetching real-time data, tying incidents to the road network."""
        state = {}
        all_nodes = list(_self.road_graph.nodes)
        for zone, data in _self.zones.items():
            incidents = []
            for _ in range(np.random.randint(0, 4)):
                node = np.random.choice(all_nodes)
                loc_coords = _self.road_graph.nodes[node]['pos'] # [lat, lon]
                incident_point = Point(loc_coords[1], loc_coords[0]) # Shapely wants (lon, lat)
...
class CognitiveEngine:
    """The 'brain' of the system, powered by XGBoost and A* graph-based pathfinding."""
    def __init__(self, data_fabric: DataFusionFabric):
        self.data_fabric = data_fabric
        self.demand_model, self.model_features = self._get_demand_model_xgboost()

    @st.cache_resource
    def _get_demand_model_xgboost(_self) -> tuple:
        """Trains and caches a high-performance XGBoost demand forecasting model."""
        hours = 24 * 365 # More data for a better model
        ...
        model_params = _self.data_fabric.config.get('model_params', {})
    ...
    def predict_citywide_demand(self, features: Dict) -> float:
        """Predicts total city-wide calls using the XGBoost model."""
    ...
    def find_best_route_for_incident(self, incident: Dict, risk_scores: Dict) -> Dict:
        """Calculates the optimal ambulance and hospital using A* pathfinding on the road graph."""
        ...
        # Find closest available ambulance by network distance
        ...
        amb_data = self.data_fabric.ambulances[ambulance_unit]

        def cost_heuristic(u, v, d):
            edge_data = self.data_fabric.road_graph.get_edge_data(u, v)
            # Find which zone the middle of the road segment is in to apply risk
            pos_u = self.data_fabric.road_graph.nodes[u]['pos']
            pos_v = self.data_fabric.road_graph.nodes[v]['pos']
            midpoint = Point(np.mean([pos_u[1], pos_v[1]]), np.mean([pos_u[0], pos_v[0]])) # lon, lat
        ...
        for name, h_node in hosp_node_map.items():
            ...
            try:
                ...
                full_path_nodes = path_to_incident + path_to_hospital[1:] # Avoid duplicating incident node
                
                load_pct = _safe_division(h_data.get('load', 0), h_data.get('capacity', 1))
                load_penalty = load_pct**2 * 20 # Significant penalty for high load
                total_score = total_eta * 0.8 + load_penalty * 0.2 # ETA is the dominant factor
        ...
        # Get coordinates [lon, lat] for pydeck path layer
        ...
        return {
            ...
            "best_hospital": best_option.get('hospital'),
            "routing_analysis": pd.DataFrame(options).drop(columns=['path_nodes']).sort_values('total_score').reset_index(drop=True),
            "route_path_coords": path_coords
        }

# --- L2: PRESENTATION LAYER ---
def kpi_card(icon: str, title: str, value: Any, color: str):
    """Renders a robust, high-impact KPI card using HTML/CSS for a dark theme."""
    st.markdown(f"""
    <div style="background-color: #262730; border: 1px solid #444; border-radius: 10px; padding: 20px; text-align: center; height: 100%;">
        ...
        <div style="font-size: 16px; color: #bbb; margin-top: 10px; text-transform: uppercase; font-weight: 600;">{title}</div>
...
def create_deck_gl_map(zones_gdf, hospital_df, ambulance_df, incident_df, heatmap_df, route_info=None, style_config=None):
    """Creates a rich, multi-layered, 3D PyDeck map with true routing and heatmaps."""
...
def display_ai_rationale(route_info: Dict):
    """Generates a plain-English explanation for the AI's A* routing decision."""
...
def main():
    ...
    if tab_choice == "Live Operations":
        ...
        with map_col:
            ...
            clicked_state = st.pydeck_chart(deck, key="deck_map", use_container_width=True)
        ...
        with forecast_col:
            ...
            with st.spinner("Calculating 24-hour forecast with XGBoost..."):
                ...
                for ts in future_hours:
                    ...
                    mean_pred = engine.predict_citywide_demand(features); std_dev = mean_pred * 0.10 # XGBoost is often more confident
        ...
        with feature_col:
            ...
            st.info("This chart shows which factors have the biggest impact on our call demand forecast. It's the 'why' behind the AI's prediction.")
        ...
        st.info("Test system resilience by simulating extreme conditions.")
        ...
        if st.button("Run Simulation", use_container_width=True):
            ...
            for zone, s_data in data_fabric.zones.items():
                l_data = live_state.get(zone, {})
                sim_risk = (l_data.get('traffic', 0.5) * sim_traffic_spike * 0.6 + 
